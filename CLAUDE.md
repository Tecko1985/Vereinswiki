...
`connect-screen` bis Login, dann `app-shell` mit vier Tabs: **Fragen** (Textarea → `askWiki` → Antwort-Karte, Strg+Enter sendet), **Dokumente** (Upload PDF/TXT + Liste mit Ansehen/Löschen, Warnbox „keine Personendaten"), **Info** (Beschreibung + Versionshistorie), **Einstellungen** (Speicherort). Antworttext wird escaped + `\n`→`<br>` gerendert (kein Markdown, Gemini wird auf Klartext instruiert). Gemeinsame CSS-Basis wie die anderen Tools; Vereinswiki-Klassen (`warn-card`, `upload-row`, `dok-row`, `answer-card`) am Ende von `style.css`.
